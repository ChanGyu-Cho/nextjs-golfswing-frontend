  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      ...
      try {
        ...
        if (jobId) ws.send(JSON.stringify({ action: "register", job_id: jobId }));
    ...
    ws.onmessage = async (ev) => {
      try {
        ...
        const url = (data.result_url || data.resultUrl || data.result) as string | undefined;
        if (url) {
          setResultUrl(url);
          ...
          try {
            ...
            try {
              ...
              if (url.match(/\.dat(\?|$)/)) {
                const urlJson = url.replace(/\.dat(\?)/, '.json$1').replace(/\.dat$/, '.json');
                try {
